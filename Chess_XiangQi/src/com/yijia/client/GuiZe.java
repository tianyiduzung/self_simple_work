...
public class GuiZe
{
	...
	//判断将军
	public boolean jiangjun(Color color){
		jiangjun=false;
		for(int i=0;i<9;i++){
			for(int j=0;j<10;j++){
				if(qiZi[i][j]!=null){
					if(color.equals(XiangQi.color1)){
						if(qiZi[i][j].getColor().equals(XiangQi.color1)){
							String name1=qiZi[i][j].getName();
							for(int k=0;k<9;k++){
								for(int l=0;l<10;l++){
									if(canMove(i,j,k,l,name1)){
										if(qiZi[k][l]!=null){
											if(qiZi[k][l].getName().equals("將")){
					...
					}else if(color.equals(XiangQi.color2)){
						if(qiZi[i][j].getColor().equals(XiangQi.color2)){
							String name1=qiZi[i][j].getName();
							for(int k=0;k<9;k++){
								for(int l=0;l<10;l++){
									if(canMove(i,j,k,l,name1)){
										if(qiZi[k][l]!=null){
											if(qiZi[k][l].getName().equals("帥")){
		...
		/*if(canMove(endI,endJ,i,j,name)){
			if(qiZi[i][j]!=null&&qiZi[i][j].getName().equals("將")){
				jiangjun=true;
				break;
			}
		}
			...
			for(int i=0;i<9;i++){
				for(int j=0;j<10;j++){
					if(canMove(endI,endJ,i,j,name)){
						if(qiZi[i][j]!=null&&qiZi[i][j].getName().equals("帥")){
							jiangjun=true;
							break;
						}
					}
