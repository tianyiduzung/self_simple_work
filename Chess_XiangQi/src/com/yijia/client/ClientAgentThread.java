...
public class ClientAgentThread  extends  Thread{
	...
	private void send(String msg) {
		...
		try {
			...
			String name1=ms[1];
			String text=ms[2];
			this.father.jta .append(name1+":"+text+"\n");
	...
	public void server_down(){
		...
		this.father.jbNChallenge.setEnabled(false);//将"拒绝挑战"按钮设为不可用
		this.father.jbFail.setEnabled(false);//将"认输"按钮设为不可用
		this.father.jta1.setEditable(false);
	...
	//注意，此处不再处理tiaoZhanZhe给它赋值，因为发出挑战时已经赋值了
	public void tong_yi(){
		...
		this.father.jbNChallenge.setEnabled(false);//将"拒绝挑战"按钮设为不可用
		this.father.jbFail.setEnabled(!false);//将"认输"按钮设为不可用
		this.father.jta1.setEditable(true);
	...
	public void move(String msg){
		...
		JOptionPane.showMessageDialog(this.father,"对面已走棋","提示",
